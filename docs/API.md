...
- **Production (deploy จริงบน Vercel):** `https://project-file-tagging.vercel.app`
- **Local (รันเว็บเองในเครื่อง):** `http://localhost:3000`

URL นี้คงที่แล้ว (ไม่ใช่ tunnel ชั่วคราวเหมือนก่อนหน้านี้อีกต่อไป) — mobile ตั้งค่าไว้ที่ `lib/config.ts` เรียบร้อยแล้ว ไม่ต้องคอยอัปเดตตามเวลามีใครปิด/เปิด dev server เหมือนเดิม

> ⚠️ ตอนนี้ pipeline สแกนไวรัส (worker ที่ทำให้ไฟล์ที่อัปโหลดกลายเป็นไฟล์จริง) ยังไม่ได้รันอยู่ที่ไหน (แผนคือรันแยกบน Railway) — อัปโหลดไฟล์ผ่าน `POST /api/files` ได้ปกติ (ได้ 202 กลับมา) แต่ไฟล์จะค้างที่สถานะ "กำลังตรวจสอบ" ไม่กลายเป็นไฟล์จริงจนกว่าจะตั้ง worker เสร็จ — ฟีเจอร์อื่นทั้งหมด (login/register/tags/ดูไฟล์เก่า) ใช้งานได้ปกติแล้ว
